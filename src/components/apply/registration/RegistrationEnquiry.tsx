...
const RegistrationEnquiry = () => {
  ...
  const { data, mutate, isValidating } = useSWRFetch<dataType>(
    `register-enquiry?perPage=10&pageNo=${pageNo}&resolved=${resolved}`
  );
  console.log(data, "kjxdgfsdjhgsdj");
        ...
        if (result.isConfirmed)
          new Promise(async (resolve, reject) => {
            const response = await enquiry({
              path: `register-enquiry/delete/${id}`,
              method: "DELETE",
            });
              ...
              >
                <div className="flex items-center gap-4">
                  {isValidating ? (
                    ...
                    <Avatar
                      className="!h-12 !w-12"
                      src={item?.user?.passportSizePhotoUrl}
                    />
                  )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isValidating ? (
                    ...
                    <span
                      ...
                      onClick={() =>
                        push(
                          `/panel/superadmin/registations/registration-enquiry/${item?._id}`
                        )
                      }
                    >
                      <Quickreply />
