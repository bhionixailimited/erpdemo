...
const EnquiryMessage = () => {
  ...
  const { enquiryId, subject } = query;

  const {
    data,
    isValidating,
    mutate: reloadEnquiry,
  } = useSWRFetch<SupportMessageDataType>(enquiryId && `enquiry/${enquiryId}`);
  ...
  const handleMarkAsResolved = async () => {
    try {
      ...
      push(`/panel/${user?.role?.toLowerCase()}/enquiry`);
    } catch (error) {
      if (error instanceof Error) {
        notify.error(error?.message);
      } else {
...
export default withProtectedSuperAdmin(EnquiryMessage);
