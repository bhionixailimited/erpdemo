...
const UpdatingResourcePerson = ({
  ...
  const { data, isValidating, mutate, error } = useSWRFetch<any>(
    `marketing-person-resource/${resourcePersonId}`
  );
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      addedByWhom: data?.data?.addedByWhom || "",
      personDesignation: data?.data?.personDesignation || "",
    ...
    onSubmit: async (values) => {
      ...
      formdata.append("addedByWhom", values?.addedByWhom);
      formdata.append("personDesignation", values?.personDesignation);
      formdata.append("organizationType", values?.organizationType);

      try {
        const response = await addVehicle({
          path: `marketing-person-resource/${resourcePersonId}`,
          method: "PUT",
          body: formdata,
          isFormData: true,
        });
      ...
      <CustomDialog open={open} onClose={handleClose} maxWidth="md">
        <div className="w-full p-4 ">
          <h3 className="font-semibold tracking-wide p-4  text-theme border-b text-2xl text-center">
            Update Marketing Resource Person
          </h3>
...
export default UpdatingResourcePerson;
