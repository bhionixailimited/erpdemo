...
const InventoryCategory = () => {
  ...
  const { data, isValidating, error, mutate } =
    useSWRFetch<dataType>("inventory/category");
        ...
        if (result.isConfirmed)
          new Promise(async (resolve, reject) => {
            const response = await designation({
              path: `inventory/category/delete/${id}`,
              method: "DELETE",
            });
  ...
  return (
    <div className="w-full">
      <PrivateLayout title="Set Inventory Category | Manage">
        <div className="px-4 mt-4">
          <div className="mb-4 flex bg-indigo-50 p-4 rounded-md shadow-md justify-between">
            ...
            <div className="">
              <AddInventoryCategoryDialog feeName="Add" Dmutate={mutate} />
            </div>
          </div>
          <MaterialTable
            components={{
...
export default withProtectedAdmin(InventoryCategory);
