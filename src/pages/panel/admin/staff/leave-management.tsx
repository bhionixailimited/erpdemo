...
const LeaveManagement = () => {
  ...
  const handleAcceptLeave = async (id: string, status: boolean) => {
    try {
      Swal.fire({
        title: "Are you sure?",
        text: `You want to ${
        ...
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: `Yes, ${status ? "accept it!" : "reject it!"}`,
      })
        .then(async (result) => {
          if (result.isConfirmed)
            new Promise(async (resolve, reject) => {
      ...
      <PrivateLayout title="Staff | Leave">
        <div className="m-auto px-5 py-4">
          <div className="mb-4 flex flex-col md:flex-row gap-2 bg-indigo-50 p-4 rounded-md shadow-md justify-between">
            ...
            <div>
              <AddLeaveDialog mutate={tableRef} />
            </div>
          </div>
          <MaterialTable
            components={{
            ...
            data={async (query) =>
              new Promise(async (resolve, reject) => {
                const result = fetchData({
                  method: "GET",
                  path: `staff/leave?self=false&perPage=${
                    query.pageSize
                  }&pageNo=${query.page + 1}&${
                  ...
                  <div className="flex flex-row gap-3 items-center">
                    {row?.status === "ACCEPTED" ? (
                      ...
                      <span className="flex flex-row gap-4">
                        <Tooltip title="Click to accept leave">
                          <Button
                            className="bg-green-700 text-xs font-semibold"
                            startIcon={<Verified />}
                            onClick={() => handleAcceptLeave(row?._id, true)}
                          >
                            ACCEPTED
                          </Button>
                        </Tooltip>
                        <Tooltip title="Click to reject leave">
                          <Button
                            className="bg-red-700 text-xs font-semibold"
                            startIcon={<Cancel />}
                            onClick={() => handleAcceptLeave(row?._id, false)}
                          >
                            REJECTED
                          </Button>
                        </Tooltip>
...
export default withProtectedAdmin(LeaveManagement);
