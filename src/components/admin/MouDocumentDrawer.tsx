...
import { AddAdmissionDocumentDialog, AddMouDocumentDialog } from "./dialog";
...
const MouDocumentDrawer = ({ open, onClose, Dmutate, feeName }: any) => {
  ...
  const {
    ...
    mutate: mouMutate,
    isValidating,
  } = useSWRFetch<mouType>(open && `mou/${open}`);
        ...
        >
          <div className="w-full p-4 ">
            ...
            <div className="w-full p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4">
              ...
              ) : mou?.data && mou?.data?.doc?.length > 0 ? (
                mou?.data?.doc?.map((item) => (
                  <MouDocumentCard
                    _id={item?._id}
                    img={item?.url}
                    key={item?._id}
