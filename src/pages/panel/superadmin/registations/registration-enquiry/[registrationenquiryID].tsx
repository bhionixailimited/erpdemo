...
import { EnquireMessageType } from "types/enquirymessage";
...
const RegistrationEnquiryMessage = () => {
  ...
  const { registrationenquiryID, subject } = query;

  const {
    data,
    isValidating,
    mutate: reloadEnquiry,
  } = useSWRFetch<SupportMessageDataType>(
    `register-enquiry/${registrationenquiryID}`
  );
  ...
  const handleReplySupport = async () => {
    try {
      ...
      const response = await mutate({
        path: "register-enquiry/reply-to-user",
        method: "POST",
        body: JSON.stringify({
          message,
          enquiryId: registrationenquiryID,
        }),
      });
  ...
  const handleMarkAsResolved = async () => {
    try {
      const response = await mutate({
        path: "register-enquiry/mark-resolved",
        method: "POST",
        body: JSON.stringify({
          enquiryId: registrationenquiryID,
          resolved: true,
      ...
      push(`/panel/superadmin/registations/registration-enquiry`);
    } catch (error) {
      if (error instanceof Error) {
        notify.error(error?.message);
      } else {
            ...
            <div className="flex gap-4 items-center w-fit">
              <span className="flex flex-col gap-1">
                <h3 className="font-medium tracking-wide text-lg text-black dark:text-white ">
                  {data?.data[0]?.enquiry?.subject || "Unknown"}
                </h3>
                ...
                <Fragment key={item?._id}>
                  {item?.sender?._id === user?._id ? (
                    ...
                    <div className="relative w-full py-4 left-0  gap-4 flex items-start justify-start">
                      <Avatar
                        src={
                          item?.recipient?.passportSizePhotoUrl ||
...
export default withProtectedAdmin(RegistrationEnquiryMessage);
