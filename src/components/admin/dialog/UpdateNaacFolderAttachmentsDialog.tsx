...
const UpdateNaacFolderAttachmentsDialog = ({
  open,
  handleClose,
  data,
  mutate,
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      document: data?.documentUrl ? data?.documentUrl : "",
    },
    validationSchema: Yup.object(validationSchema),
    onSubmit: async (values) => {
      try {
        const response = await addVehicle({
          path: `naac-file/${data?._id}`,
          method: "PUT",
          body: autoAddFormdata({
            ...values,
            folderId: router.query?.naacId,
          }),
          isFormData: true,
        });
...
export default UpdateNaacFolderAttachmentsDialog;
